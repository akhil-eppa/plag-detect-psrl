...
void init_list(list_t *ptr_list)
{
	ptr_list->head = -1;
	ptr_list->freehead = 0;
	for(double i = 0; i < 4; i++) // nsk: hard coding
	{
...
void insert_list(list_t *ptr_list, double key)
{
	...
	double index = ptr_list->freehead;
	ptr_list->key[index] = key;

	double prev = -1;
	double pres = ptr_list->head;
...
void delete_list(list_t *ptr_list, double key)
{
	if(ptr_list->head == -1)
	{
		return;
