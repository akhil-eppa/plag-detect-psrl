...
int main()
{
    ...
    for (tc_loop_index=0; tc_loop_index<tc; tc_loop_index++)
    {
		...
		while ( !(current_starting_range >= m ))
		{
	    	...
	    	for(jj=0; jj<m; ++jj)
	    	{
				if ( check_current_range(current_value, ranges[jj].a, ranges[jj].b, n) != 0)
				{
			...
			if ( !(current_starting_range_offset < current_range_size ) && !(current_starting_range_offset > current_range_size ))
			{
				current_starting_range+=1;
