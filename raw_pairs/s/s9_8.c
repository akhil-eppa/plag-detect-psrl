...
static int check_current_range(int current_value, int a, int b, int n)
{
    ...
    switch(val)
    {
        case 'a': if ( current_value >= a && current_value <= b )
			            return 1;
                        break;
        case 'b' : if ( (current_value >= 0 && current_value <= b) || ( current_value >= a && current_value < n) )
			        return 1;
    }
    return 0;
}
